...
| ~~`conn_noise`~~ | — | **开关已移除，恒开**：段间一律做锥形重绘（见「段间引导」）。旧工作流的该档位会在载入时被丢弃 |
...
| `asr_model` | 可选 | 音频有效性校验：接上后运行一次，r2v 工具栏的「音频有效性校验」按钮即可反复核对 |
...
- 段间锥形重绘**恒开**（原 `conn_noise` 开关已移除）：把上一段尾写入本段 body 前缀并重绘（continue 模式），重绘幅度由时间轴面板的「重绘幅度」控制，默认 0.10（0 = 接缝硬锁，0.95 = 几乎不重绘）。
...
**提取音频**：从源视频抽出一段 PCM，存在 `<节点缓存目录>/audio_extract/`，按 **entry id** 命名（不是内容哈希、不是时间轴位置），因此改提示词 / 重跑都不会失效，「清空缓存」也不会误删。片段重排时条目跟着卡片走，卡片删除时条目一并删除。**每段每来源只保留一条**（`(seg_id, variant)` 唯一，所以对同一段分别提取一采/二采会各有一条）：重新提取同一个片段是**覆盖**（沿用原 id、就地覆盖文件），不会堆积；**磁盘上已有的老条目也会自动收敛**——打开「音频」tab / 删除卡片时会丢弃重复项（被「保留音频」钉住的那条优先，否则留最新）并删掉它们的文件。且批量面板里已有入口时，主工具栏的同名按钮会自动隐藏，避免两个按钮。要一次清掉所有提取音频，用「清空节点所有缓存」。

某段提取不出来时，结果区会**逐段写明原因**：该来源还没渲染（`no-cache`）／只有 AV latent 缓存、HTTP 里无法解码（`latent-only`）／渲染缓存里没有音轨、可能是静音输出（`no-audio-track`）／缓存读不出来（`unreadable`）／该段不在后端解析出的时间轴里（`not-in-plan`）。

**保留音频**：卡片头部（「引用上段」旁边）的开关。打开后，运行 Phase 2 会把该段提取出来的音频 VAE 编码进 AV latent 的音频流并把该流的 `noise_mask` 置零 —— UNet 看得到它、以它为条件，但永远不会重绘它；Phase 3 跳过音频 VAE 解码，直接把同一份 PCM 混流回去，所以听到的就是提取的那一段，不经过音频 VAE 往返。保留的是该段最近一次提取的音频；没提取过会提示先去提取。

---

## 音频有效性校验（ASR）

`asr_model`（来自 `Comfyui-MOSS-Transcribe-Diarize-T8` 的 `T8_MOSS_ModelLoader`，类型 `T8_MOSS_TRANSCRIBE_MODEL`）接上后会沿用你在加载器上选的设备/精度；**不接也能用**——后端会用该加载器自己的默认参数构造句柄，所以**不需要为了校验跑一次节点**。**r2v 工具栏**「提取音频」旁边就是「**音频有效性校验**」按钮：

1. 点按钮 → 先选缓存来源（一采 / 二采），再勾选要核对的片段（没有音轨缓存的片段置灰不可选）；
2. 确认后，后端读取这些片段**已缓存**的音轨，与本段**当前**提示词里的台词块逐说话人比对（错误率 + 说话人是否对得上）；
3. 结果写进每张卡片预览区的「**识别**」页签：**本段**提示词台词 vs **本段**识别到的内容，下面再附整批判定；不写入 `report`，也**不会重新生成任何片段**。
...
| `audio_extract/` | 提取音频（独立生命周期；只有「清空节点所有缓存」会连它一起清） |
...
- **清空缓存**：删文本编码缓存 + batch 临时文件 + 旧版 `*_frames_ht.pt`，保留已渲染片段，**也不动「提取音频」导出的条目**。
- **清空节点所有缓存**：额外删除所有 `seg_*`（一采）文件 **以及整个 `audio_extract/` 音频库**（含 `index.json`），强制全部重算、音频也要重新提取；同时会解掉时间轴上「保留音频」的指向（那些条目已经不存在了）。
...
| POST | `/asr_check_status` | 「音频有效性校验」可用性：哪些片段在该缓存来源下有音轨、模型是否就绪 |
| POST | `/asr_check` | 按当前提示词校验选中片段的已生成音轨（不重新生成） |
...
| 帧率莫名其妙变成 240 | 240 是控件上限，说明有人喂了一个 >240 的值（最常见是源视频探测：容器把**时间基** 1000/1 当帧率，VFR 的 `avg_frame_rate` 则是 0/0）。现在探测值先用「帧数 / 时长」交叉验证，不可信就不写入并打印告警；帧率被夹取时也会打印**原值**，控制台一搜即知来源 |
| 二采没接 `upscale_model` | 直接返回错误报告，不会静默出原分辨率 |
| 旧工作流打开后控件值错位 | 前端会剥除废弃输出 `segment_images`、修正失效控件值；时间轴宽高 / 总帧数异常时后端还会夹回安全默认值并告警，重新保存一次即可固化 |
| 某几个控件**每次刷新都变回默认值**（如 `use_sigmas` / `second_seed`） | 这份存档是在**值已错位**的状态下保存的：坏值被 `widgets_values_named` 按名字固化，于是每次载入都忠实还原。载入时会检测并治愈（分组标题 `bd_grp_*` 里存的不是它自己的标题即为证据 → 从该处起复位为默认值，日志会写明复位了哪些控件），改完**保存工作流一次**就不再复发 |
