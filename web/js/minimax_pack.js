/** MiniMax H3 Director Opt — director pack (script + assets) import/export UI.

Uses the Opt HTTP routes (/minimax/director_opt/*) but keeps the upstream pack
format, so packs exported by the original ComfyUI_MiniMaxH3_Director plugin
...
function downloadBlob(blob, filename) {
    ...
    a.href = url;
    a.download = filename || "MiniMaxH3DirectorOpt.mmxpack.zip";
...
export async function exportDirectorPack(editor) {
    ...
    const preview = await postJson("/minimax/director_opt/export_pack", {
        timeline,
        widgets,
        dryRun: true,
    });
    ...
    const resp = await api.fetchApi("/minimax/director_opt/export_pack", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export async function importDirectorPack(editor) {
    ...
    if (file.size <= CHUNK_SOFT_LIMIT) {
        ...
        data = await postJson("/minimax/director_opt/import_pack", {
    ...
    const bind = (sel, fn) => {
        ...
        el.onclick = async (e) => {
            ...
            try {
                ...
                console.error("[MiniMax H3 Director Opt] pack:", err);
