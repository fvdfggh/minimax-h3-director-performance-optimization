...
from ..director.asr_check import ASR_MODEL_TYPE
from ..director.asr_runtime import remember_asr_model
...
class MiniMaxH3DirectorOpt:
        ...
        return {
            ...
            "optional": {
                ...
                **director_perf_inputs(),
                ...
                # asr_model 是 forceInput 的自定义口：它不占 widget 下标，所以增删它不
                # 会影响旧工作流的 widgets_values 对齐。
                "asr_model": (
                    ASR_MODEL_TYPE,
                    {
                        "forceInput": True,
                        "tooltip": (
                            ...
                            "接线后运行一次节点，r2v 模式下的「音频有效性校验」按钮即可"
                            "反复用它核对已生成的音轨（不需要重新生成）。"
                        ),
                    },
                ),
                # 「音频有效性校验」不再是 BOOLEAN：r2v 模式下节点上会出现一个按钮，
                # 点击后选片段、按当前提示词核对已生成音轨（见 routes_asr）。
            },
    ...
    def execute(
        ...
        **kwargs,
        ...
        # Park the handle for the「音频有效性校验」button: it runs from an HTTP
        # request and cannot see this socket, so it borrows whatever we saw here.
        remember_asr_model(unique_id, workflow_name, asr_model)
